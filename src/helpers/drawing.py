...
def show_eNodeBs_cars(cars_xx: List[float], cars_yy: List[float]):
    ...
    for index, enb in enumerate(enbs):
        ...
        ax.text(enb.location.x, enb.location.y, enb.Id)
...
def save_sim_image(xx: List[float], yy: List[float], eNodeBs: List[eNodeB], image_filename: str):
    colors = [np.random.rand(3, ) for _ in range(len(eNodeBs))]

    fig, ax = plt.subplots()

    ax.plot(xx, yy, 'o')

    for index, enb in enumerate(eNodeBs):
        poly = Poly(enb.boundary_points, fill=True, color=colors[index])
        ax.add_patch(poly)

        ax.text(enb.location.x, enb.location.y, f'enb: {enb.Id}')
